...
def compute_attention_map(digit_imgs, att_map_dir, out):
                ...
                if digit == 2:
                    ...
                    while i < len(img_att_map_files):
                        ...
                        if 'ty' in cur_map_file or 'teen' in cur_map_file or 'ten' in cur_map_file or 'eleven' in cur_map_file or 'twelve' in cur_map_file:
                            added_maps[2]['pre_tys'] += np.load(os.path.join(model_path, img_att_map_files[i-1]))
                    ...
                    while i < len(img_att_map_files):
                        ...
                        if 'hundred' in cur_map_file:
                            ...
                            added_maps[3]['pre_tys'] += np.load(os.path.join(model_path, img_att_map_files[i-1]))
                    ...
                    while i < len(img_att_map_files):
                        ...
                        if 'hundred' in cur_map_file:
                            ...
                            added_maps[4]['pre_tys'] += np.load(os.path.join(model_path, img_att_map_files[i-1]))
                        elif i == len(img_att_map_files) - 1 and 'and' not in cur_map_file:
    ...
    digit_att_map = {2 : {'pre_tys' : [], 'ty_teens' : [], 'unit' : []}, 
                     3 : {'pre_tys' : [], 'pre-hundred' : [], 'hundred' : [], 'ty_teens' : [], 'unit' : []}, 
...
def unqualified_list(result_dir):
    ...
    for res_file in res_files:
        ...
        for index, row in res.iterrows():
            ...
            if target_num_true == target_num_pred and target_num_true != 'N/A' and target_num_pred != 'N/A':
                if not 10 <= target_num_true < 20:
                    model_valid_number[model].append(target_image)
            if foil_num_true == foil_num_pred and foil_num_true != 'N/A' and foil_num_pred != 'N/A':
    ...
    for model in model_valid_number.keys():
        for img in model_valid_number[model]:
            number = word2num(convert_number_label(str(gt_data[img])))
...
# Global variables
attention_positions = ['pre-thousand', 'thousand', 'pre-hundred', 'hundred', 'pre_tys', 'ty_teens', 'unit']
